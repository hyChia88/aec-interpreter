...
from aec_interpreter.evaluation_infra.contracts import (
    CandidateElement,
    ChatMessage,
    ContextMeta,
    EvalTrace,
...
async def run_pipeline_case(
    ...
    *,
    ...
    if precomputed_constraints and case_id in precomputed_constraints:
        # Use pre-extracted constraints from Modal eval (skip GPU inference)
        constraints = precomputed_constraints[case_id]
    elif constraints_model == "lora" and lora_extractor is not None:
    ...
    if rq2_enabled and rq2_schema and tool_by_name:
        try:
            from aec_interpreter.schema.pipeline import run_rq2_postprocess
            from aec_interpreter.evaluation_infra.contracts import RQ2Result
