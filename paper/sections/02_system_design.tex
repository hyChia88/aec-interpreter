...
(${\sim}27\%$) illustrates \citep{chiahuiyen_mscd_thesis}. Third, the end-to-end matcher is our
measured baseline, and it plateaus: the strongest fine-tuned multimodal reranker reaches Top-1 6.7\%
with the answer in the pool 100\% of the time, while the same pool reorders to 78.5\% when the
relational address is supplied (\secref{sec:eval-oracle}). Finally, construction compliance requires
...
versus 30--63\% zero-shot \citep{chiahuiyen_mscd_thesis}. The G-series ablation also exposes a
...
cases. The output is 990 training cases on the studied project, a 60-case held-out benchmark (the
evaluation set throughout \secref{sec:eval}), and a 116-case cross-IFC generalization benchmark.
